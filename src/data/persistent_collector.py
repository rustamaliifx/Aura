...
class PersistentElasticsearchService:
    ...
    async def fetch_incremental_data(self, index: str, size: int = 1000) -> int:
        ...
        if last_timestamp is None:
            # First time fetch - get data from 30 days ago
            last_timestamp = current_time - timedelta(days=30)
            logger.info("First time fetch - getting last 30 days of data")
        else:
        ...
        query = {
            "query": {
                "range": {
                    "@timestamp": {
                        "gt": start_time,  # Use 'gt' to avoid duplicates
                        "lte": end_time
                    }
                }
            },
